...
function changepass(e) {
  ...
  var oldpass =
    document.forms["profilePasswordChangeForm"]["oldPassword"].value;
  var newpass =
    document.forms["profilePasswordChangeForm"]["newPassword"].value;
  var newpassconf =
    document.forms["profilePasswordChangeForm"]["confirmNewPassword"].value;
  ...
  if (newpass !== newpassconf) {
    ...
    request.done(function (msg) {
      if (msg == "SUCCESS") {
        ...
        $("#profilePasswordChangeForm")[0].reset();
        $("#profile").modal("hide");
      } else {
        //			Error
        $("#passmsg").text(msg);
...
$(".reuploadimage").change(function (e) {
  ...
  if (this.files[0].size / 1024 > 256) {
    ...
    document.getElementById(event.target.id + "-pic").src =
...
$("#viewmfdkycdoc").on("click", function (e) {
  ...
  request = $.ajax({
    ...
    beforeSend: function () {
      //disableChangePassButton();
      $("#viewmfdkycdoc").html(
        'Fetching <i class="fas fa-spinner fa-spin"></i>'
      );
    ...
    if (msg.statusCode == 100 && msg.statusMsg == "RECORD_EXIST") {
      try {
        for (i in msg.obj1) {
          ...
          divtest.innerHTML =
            "<img class='img-fluid mx-auto' style='height: 8rem;' src='data:image/png;base64," +
            x.image +
            "'> <div class='card-body border-top'><p class='card-text'><strong>Type- </strong>" +
            x.imagesubtype +
            "</p></div> ";
  ...
  request.fail(function (jqXHR, textStatus) {
    //		console.log(textStatus);
    $("#kycdocdata").html(
      "<h6>Failed to make request. Please try after sometime</h6>"
    );
  });
